...
public class CauHoiService implements ICauHoiService {
    ...
    public void changeMedia(Long cauHoiId, String duongDanTep) throws DataNotFoundException {
        ...
        String oldFile = cauHoi.getDuongDanTep();
        if (oldFile != null && !oldFile.trim().isEmpty()) {
            try {
                com.app.backend.utils.FileUtils.deleteFile(oldFile);
            } catch (Exception e) {
                // Log lỗi nhưng không throw để không chặn việc upload file mới
                System.err.println("Không thể xóa file cũ: " + oldFile + " - " + e.getMessage());
            }
        }
    ...
    public CauHoi create(CauHoiDTO cauHoiDTO, Long currentUserId, boolean isAdmin) throws DataNotFoundException, PermissionDenyException {
        ...
        // Clear cache cho bộ câu hỏi này
        cauHoiRedisService.clearCacheForBoCauHoi(boCauHoi.getId());
        return saved;
    }
    ...
    public CauHoi update(Long id, CauHoiDTO cauHoiDTO, Long currentUserId, boolean isAdmin) throws DataNotFoundException, PermissionDenyException {
        ...
        CauHoi saved = cauHoiRepository.save(cauHoi);
        // Clear cache cho bộ câu hỏi này (và bộ cũ nếu chuyển sang bộ khác)
        cauHoiRedisService.clearCacheForBoCauHoi(cauHoi.getBoCauHoi().getId());
        return saved;
    }
    ...
    public void delete(Long id, Long currentUserId, boolean isAdmin) throws DataNotFoundException, PermissionDenyException {
        ...
        Long boCauHoiId = cauHoi.getBoCauHoi().getId();
        cauHoiRepository.delete(cauHoi);
        // Clear cache cho bộ câu hỏi này
        cauHoiRedisService.clearCacheForBoCauHoi(boCauHoiId);
    }
